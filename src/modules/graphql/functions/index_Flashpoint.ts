...
// item Detail
export async function getNftData_Flashpoint(
  ...
  try {
    const data: any = await graphQLClient.request(gql_Flashpoint.getNftData, variables);
...
type INFTFiltersNew = INFTFilters | null;

export async function getNftsData_Flashpoint(
  namespace: string, //Market namespace
  serviceID: string | number,
  seller: string,
  owner: string,
  filters: INFTFiltersNew = null,
): Promise<NftsData> {
  const ret: NftsData = {
    nfts: [],
    pageInfo: {
  ...
  try {
    const data: any = await graphQLClient.request(gql_Flashpoint.getNftsData, variables);
...
export async function getNftDataOffering_Flashpoint(
  namespace: string, //Market namespace
  serviceID: string,
  ...
  try {
    const data: any = await graphQLClient.request(gql_Flashpoint.getNftDataOffering, variables);
